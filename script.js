...
//const m = Number(inputM.value); //Počet řádků/zdrojů
//const n = Number(inputN.value); //Počet sloupců/odběratelů
...
function generator() {
	let Agen = [150, 310, 370, 330];
	let Bgen = [310, 310, 360, 180];
	let C = [
		[12, 37, 7, 39],
		[12, 49, 24, 32],
		[4, 26, 7, 15],
...
function zadani(A, B, C, zadaniZ, cisloZadani) {
...
function vam(A, B, C) {
	...
	for (let x = 0; x < m + n - 1; x++) {
		...
		for (let i = 0; i < m; i++) {
			...
			console.log("diferenceRow", diferenceRow);
			...
			console.log("hodnotaDiffRow", hodnotaDiffRow);
		}
		...
		for (let j = 0; j < n; j++) {
			...
			console.log("hodnotaDiffCol", hodnotaDiffCol);
		}
		...
		console.log("hodnotaDiff", hodnotaDiff);
		...
		if (A[i3] == 0) {
			//Pokud je vyčerpaný zdroj, eliminuj řádek
...
function displayOutput(A, B, C, zadaniZ, cisloZadani) {
	let { X } = vam(A, B, C);
...
function application() {
	...
	for (let num = 0; num < pocetZadani; num++) {
		let { Agen, Bgen, C, zadaniZ } = generator();
		let { A, B } = vyrovnavac(Agen, Bgen);
		let { output, output4 } = zadani(A, B, C, zadaniZ, cisloZadani);
		let { output5, output6 } = displayOutput(A, B, C, zadaniZ, cisloZadani);
